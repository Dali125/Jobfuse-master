...
class _RatingsState extends State<Ratings> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        width: width,
        child: StreamBuilder(
          stream: FirebaseFirestore.instance.collection('user_ratings_and_review').where('reviewed_client_id', isEqualTo: widget.userID).snapshots(),
  ...
  //The rating design goeshere
                return FutureBuilder(
                  future: FirebaseFirestore.instance.collection('users').where('Userid', isEqualTo: ratingData['reviewer_id'] ).get(),
                  builder: (context, snapshot){

                var userdata = snapshot.data?.docs[0];

                DateTime timestamp = ratingData["time_of_review"].toDate();
                    if(snapshot.hasData){
